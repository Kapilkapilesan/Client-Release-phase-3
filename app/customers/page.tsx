...
export default function CustomersPage() {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            {/* Search and Filters */}
            <div className="bg-card rounded-[2rem] border border-border-default p-4 shadow-sm">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="relative flex-1">
                        <Search className="w-4 h-4 text-text-muted/50 absolute left-4 top-1/2 -translate-y-1/2" />
                        <input
                            type="text"
                            placeholder="Search customers by name, NIC, or mobile..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-11 pr-4 py-3 bg-input border border-border-divider/50 rounded-2xl text-sm font-black tracking-tight focus:outline-none focus:ring-4 focus:ring-primary-500/5 focus:border-primary-500/30 transition-all text-text-primary placeholder:text-text-muted/30"
                        />
                    </div>
                    <div className="flex flex-wrap gap-3">
                        <div className="flex items-center gap-2 border border-border-divider/50 rounded-2xl px-4 bg-input transition-all focus-within:ring-4 focus-within:ring-primary-500/5 focus-within:border-primary-500/30">
                            <Filter className="w-4 h-4 text-text-muted/40" />
                            <select
                                value={filterStatus}
                                onChange={(e) => setFilterStatus(e.target.value as 'All' | 'active' | 'blocked')}
                                className="bg-transparent border-none text-[11px] font-black uppercase tracking-widest py-3 focus:outline-none text-text-primary cursor-pointer appearance-none"
                            >
                        ...
                        <div className="flex items-center gap-2 border border-border-divider/50 rounded-2xl px-4 bg-input transition-all focus-within:ring-4 focus-within:ring-primary-500/5 focus-within:border-primary-500/30">
                            <MapPin className="w-4 h-4 text-text-muted/40" />
                            <select
                                value={filterBranch}
                        ...
                        <div className="flex items-center gap-2 border border-border-divider/50 rounded-2xl px-4 bg-input transition-all focus-within:ring-4 focus-within:ring-primary-500/5 focus-within:border-primary-500/30">
                            <Building className="w-4 h-4 text-text-muted/40" />
                            <select
                                value={filterCenter}
                        ...
                        <div className="flex items-center gap-2 border border-border-divider/50 rounded-2xl px-4 bg-input transition-all focus-within:ring-4 focus-within:ring-primary-500/5 focus-within:border-primary-500/30">
                            <Briefcase className="w-4 h-4 text-text-muted/40" />
                            <select
                                value={filterGroup}
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-start">
                {/* Customer Table */}
                <div className="flex-1 min-w-0 bg-card rounded-[2rem] border border-border-default overflow-hidden shadow-sm">
                    {loading ? (
