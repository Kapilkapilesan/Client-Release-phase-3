    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-app-background">
            ...
            <div className="relative z-10 p-8 space-y-8 animate-in fade-in duration-700 max-h-screen overflow-y-auto no-scrollbar">
                {/* Header Section */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-card p-8 rounded-[2.5rem] shadow-xl border border-border-default relative overflow-hidden group">
                    ...
                    <div className="flex items-center gap-6 relative z-10">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-3">
                                <span className="text-[11px] text-text-muted font-extrabold uppercase tracking-[0.2em] flex items-center gap-2">
                                ...
                                <span className="text-[11px] text-text-muted font-bold uppercase tracking-widest">Institutional Trace Verified</span>
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center gap-4 relative z-10">
                        <div className="flex items-center gap-2 bg-muted-bg p-2 rounded-2xl border border-border-default shadow-inner group/period">
                            {['day', 'month', 'year', 'all'].map((period) => (
                                <button
                                    ...
                                    className={`px-5 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all duration-300 ${selectedPeriod === period
                                        ? 'bg-card text-primary-600 shadow-xl scale-105'
                        ...
                        <div className="h-10 w-px bg-border-default hidden md:block" />
                        <div className="relative group/date">
                            <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-text-muted group-focus-within/date:text-primary-500 transition-colors" />
                            <input
                                type="date"
                                value={selectedDate}
                                onChange={(e) => setSelectedDate(e.target.value)}
                                className="pl-12 pr-6 py-3.5 bg-card border border-border-default rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none ring-4 ring-primary-500/0 focus:ring-primary-500/5 focus:border-primary-500/50 transition-all w-52 text-text-primary shadow-sm"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-card rounded-[3rem] border border-border-default shadow-2xl overflow-hidden flex flex-col min-h-[600px]">
                    <div className="flex flex-wrap items-center bg-muted-bg/30 px-8 pt-8 gap-4 border-b border-border-default">
                        {[
                            ...
                            >
                                <tab.icon className={`w-4 h-4 transition-transform group-hover:scale-110 ${activeTab === tab.id ? 'text-primary-500' : 'text-text-muted/50'}`} />
                                {tab.label}
                                {activeTab === tab.id && (
                        ...
                        {isAdmin && (
                            <div className="ml-auto mb-8 flex items-center gap-4 px-6 py-2.5 bg-card rounded-2xl border border-border-default shadow-sm group/branch">
                                ...
                                <select
                                    ...
                                    className="bg-transparent border-none outline-none font-black text-[10px] uppercase tracking-widest text-text-primary cursor-pointer min-w-[180px]"
                                >
                            ...
                            <div className="space-y-8">
                                <div className="flex justify-between items-center px-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">Liquidation Status Management</p>
                                    </div>
                                    <div className="bg-muted-bg p-1.5 rounded-2xl border border-border-default shadow-inner overflow-hidden">
                                        {[
                    ...
                    <div className="px-12 py-6 bg-muted-bg/20 border-t border-border-default flex items-center justify-between opacity-60">
                        <div className="flex items-center gap-4">
                            <p className="text-[9px] text-text-muted font-bold uppercase tracking-[0.4em]">
