...
export const useLoanForm = () => {
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                ...
                const filteredProducts = (productsData || []).filter((p: any) => p.product_type === 'micro_loan' || !p.product_type);
        ...
        setFormData((prev) => {
            ...
            if (field === 'loanProduct') {
                ...
                if (product) {
                    ...
                    // Use first term's interest rate and tenure if terms exist
                    if (product.product_terms && product.product_terms.length > 0) {
                        const firstTerm = product.product_terms[0];
                        newData.tenure = firstTerm.term.toString();
                    ...
                    if (t === 48) newData.processingFee = (amount * 0.04).toFixed(2);
                    else if (t === 72) newData.processingFee = (amount * 0.06).toFixed(2);
            ...
            if (field === 'tenure' || field === 'loanAmount') {
                ...
                if (tenure === 48) newData.processingFee = (amount * 0.04).toFixed(2);
                else if (tenure === 72) newData.processingFee = (amount * 0.06).toFixed(2);
                else newData.processingFee = '0';
            }
    ...
    const loadFromLoan = useCallback((loan: Loan) => {
        setFormData({
            ...
            rentalType: loan.product?.term_type?.toLowerCase().includes('month') ? 'Monthly' : 'Weekly',
