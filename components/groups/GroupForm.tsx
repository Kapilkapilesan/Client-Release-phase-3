...
export function GroupForm({ isOpen, onClose, onSubmit, initialData }: GroupFormProps) {
    ...
    useEffect(() => {
        const loadCustomers = async () => {
            ...
            try {
                // Fetch only Loan Customers for this specific center
                const customers = await customerService.getCustomers({
                    center_id: selectedCenterId,
                    customer_type: 'Loan Customer'
                });

                // Filter: Active Loan Customers + (not in any group OR already in this group if editing)
                const filtered = customers.filter(c =>
                    c.status === 'active' &&
                    c.customer_type === 'Loan Customer' &&
                                        ...
                                        filteredCustomers.map((customer) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`p-3.5 flex items-center justify-between cursor-pointer hover:bg-muted-bg transition-colors ${isSelected ? 'bg-blue-500/5' : ''} ${activeLoan ? 'opacity-60 cursor-not-allowed bg-muted-bg/50' : ''}`}
                                                >
