        ...
        {isLoadingData ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="sticky bottom-0 bg-card pt-4 border-t border-border-divider flex gap-3 justify-end transition-colors pb-2">
              ...
              <button
                type="submit"
                disabled={!!centerNameDuplicateError || isCheckingDuplicate || (noBranchAssigned && !initialData)}
                className={`px-6 py-2 rounded-xl font-black text-sm transition-all shadow-lg active:scale-95 flex items-center gap-2 ${centerNameDuplicateError || isCheckingDuplicate || (noBranchAssigned && !initialData)
                  ? "bg-muted text-text-muted cursor-not-allowed grayscale"
                  : "text-white hover:opacity-90"
                  }`}
                style={!(centerNameDuplicateError || isCheckingDuplicate || (noBranchAssigned && !initialData)) ? { backgroundColor: colors.primary[600] } : {}}
              >
                {initialData
