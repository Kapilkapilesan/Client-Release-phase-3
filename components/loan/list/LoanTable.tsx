...
export function LoanTable({ loans, onView, onDelete }: LoanTableProps) {
    ...
    return (
        <div className="bg-card rounded-2xl border border-border-default overflow-hidden transition-colors">
                        ...
                        <div key={loan.id} className="px-6 py-4 hover:bg-hover transition-colors group">
                            <div className="grid grid-cols-12 gap-4 items-center">
                                ...
                                <div className="col-span-2">
                                    ...
                                    <p className="text-[10px] font-black text-text-muted uppercase tracking-widest opacity-60">{loan.agreement_date || 'N/A'}</p>
                                </div>
                                ...
                                <div className="col-span-2">
                                    <div className="flex items-center justify-between mb-1.5">
                                        ...
                                        <span className="text-[9px] font-black text-text-muted">
                                            {loan.approved_amount && Number(loan.approved_amount) > 0 ? `${Math.round(((Number(loan.approved_amount) - Number(loan.outstanding_amount)) / Number(loan.approved_amount)) * 100)}%` : '0%'}
                                        </span>
                                    </div>
